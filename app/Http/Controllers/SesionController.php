...
class SesionController extends Controller
{
    ...
    */
    ...
    public function crearListaInsertarAsistencia($listaMiembros,$idEvento)
    {
        ...
        {
            $listaRetorno2 = ['id_usuario' => (int)$miembro->idmiembro,
    ...
    public function iniciarSesion($id,Request $request)
    {
        if($this->acceso($request))
        {
            ...
            if (sizeof($puntos)>0)
            {
                ...
                // return $puntoActivo;
                $listaMiembrosConvocados = $sesion->obtenerMiembrosConvocados((int)$id);
                return view('sesion.inicioSesion',['sesion'=>$sesion2,'puntosAgenda'=>$puntos,'puntoActivo'=>$puntoActivo,'largoPuntos'=>$largoPuntos,'miembro'=>$listaMiembros, 'votacionAbierta'=>$valorConsultaVotacion, 'abierta'=>$valorMandar,'votacion'=>$votacion,
